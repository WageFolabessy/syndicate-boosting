...
class RankTierController extends Controller
{
    public function index(Request $request)
    {
        ...
        return DataTables::of($rankTiers)
            ...
            ->editColumn('created_at', function ($tier) {
                return $tier->created_at
                    ? $tier->created_at->locale('id')->translatedFormat('l, d F Y, H:i:s')
                    : '';
            })
            ->editColumn('updated_at', function ($tier) {
                return $tier->updated_at
                    ? $tier->updated_at->locale('id')->translatedFormat('l, d F Y, H:i:s')
                    : '';
            })
            ->addColumn('action', function ($tier) {
                return view('dashboard.pages.rank-tier.action-button', compact('tier'))->render();
            })
            ->rawColumns(['action'])
            ->make(true);
    }
    ...
    public function store(AddRankTierRequest $request)
    {
        ...
        return redirect()->route('dashboard.rank-tier')
            ->with('success', 'Rank Tier berhasil ditambahkan.');
    }
    ...
    public function update(UpdateRankTierRequest $request, GameRankTier $rankTier)
    {
        ...
        return redirect()->route('dashboard.rank-tier')
            ->with('success', 'Rank Tier berhasil diperbarui.');
    }
